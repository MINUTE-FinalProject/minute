...
function ShortsVideoPage() {
  ...
  useEffect(() => {
    ...
    // incomingList가 없거나 비어있으면 DB/API에서 기본 목록 가져오기
    console.log("DEBUG: useEffect[9] - incomingList가 없거나 비어서 DB/API에서 기본 목록 가져오기 시도.");
    const dbFetch = axios.get(`/api/v1/youtube/db/shorts?maxResults=15`)
        .then(res => res.data) // axios는 res.data로 접근
        .catch(() => { console.error("DB shorts fetch 실패"); return []; });

    // YouTube API 직접 호출은 예시로만 남겨둡니다. 실제로는 백엔드를 통하는 것이 좋습니다.
    // const apiFetch = axios.get(`/api/v1/youtube/shorts?maxResults=15`)
    //   .then(res => res.data)
    //   .catch(() => { console.error("API shorts fetch 실패"); return []; });
    // Promise.all([dbFetch, apiFetch]).then(([dbVideos, apiVideos]) => {

    dbFetch.then(dbVideos => {
      ...
      console.log("DEBUG: useEffect[9] - DB/API에서 가져온 최종 목록:", JSON.parse(JSON.stringify(allItems)));
      setOriginalShorts(allItems);

      let initialIdx = 0;
      if (paramVideoId && allItems.length > 0) {
      ...
      console.log("DEBUG: useEffect[9] - DB/API 목록 사용 시, 계산된 initialIdx:", initialIdx);
      if (currentOriginalIdx !== initialIdx) {
        setCurrentOriginalIdx(initialIdx);
      } else if (allItems.length > 0 && playerRef.current && playerRef.current.loadVideoById) {
         const videoToLoad = allItems[initialIdx]?.id?.videoId || allItems[initialIdx]?.videoId;
         if (videoToLoad) {
            console.log("DEBUG: useEffect[9] - 인덱스 동일, 강제 비디오 로드 (DB/API):", videoToLoad);
            // playerRef.current.loadVideoById({ videoId: videoToLoad });
         }
      }
    }).catch(err => {
        console.error("DEBUG: useEffect[9] - DB/API shorts 목록 가져오기 최종 실패:", err);
        setOriginalShorts([]); // 실패 시 빈 배열로 설정
    });
  }, [paramVideoId, incomingListFromState, pageOriginFromState]); // location.state의 내용이 바뀔 때마다 실행
  ...
  useEffect(() => {
    ...
    if (!originalShorts.length || currentOriginalIdx < 0 || currentOriginalIdx >= originalShorts.length) return;
  ...
  }, [currentOriginalIdx, originalShorts, isLoggedIn]); // 현재 영상이 바뀌거나 로그인 상태 변경 시
  ...
  const handleThumbUpClick = async () => {
    ...
    try {
      if (!isNowLiked) {
        await axios.post(`/api/v1/auth/${userId}/videos/${currentVideoId}/like`, null, { headers: { Authorization: `Bearer ${token}` } });
        ...
        await axios.delete(`/api/v1/auth/${userId}/videos/${currentVideoId}/like`, { headers: { Authorization: `Bearer ${token}` } });
        setLikes((prev) => { const copy = { ...prev }; delete copy[currentVideoId]; return copy; });
      }
    } catch (err) { console.error("좋아요 API 에러", err); }
  };
